...
from tests.conftest import _ADMIN_USER, _REGULAR_USER, FakeMailsStore, _make_fake_check_document_exists, _make_fake_update_document

_test_app = FastAPI()
_test_app.include_router(router, prefix="/api/platform/tools")

FEATURE = "../../../../features/platform/tools/mail/update_mail.feature"


@scenario(FEATURE, "PUT /mail/0010 with valid body as admin — the mail is updated")
def test_update_admin():
    pass


@scenario(FEATURE, "PUT /mail/0010 as a viewer — 403 error and the mail is not modified")
...
def admin_client(mails_store: FakeMailsStore):
    ...
    with (
        ...
        patch("app.platform.tools.mail.router.update_document",
              new=AsyncMock(side_effect=_make_fake_update_document(mails_store))),
    ):
