...
def admin_client(managed_users_store: FakeUsersStore):
    ...
    with (
        ...
        patch("app.platform.functions.people.users.router.check_own_user_or_admin",
              new=AsyncMock(return_value=None)),
        patch("app.platform.functions.people.users.router.check_document_exists",
              new=AsyncMock(side_effect=_make_fake_check_document_exists(managed_users_store))),
        patch("app.platform.functions.people.users.router.check_unique_field",
              new=AsyncMock(return_value=None)),
        patch("app.platform.functions.people.users.router.update_document",
              new=AsyncMock(side_effect=_make_fake_update_document(managed_users_store))),
    ):
