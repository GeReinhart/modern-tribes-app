...
from tests.conftest import _ADMIN_USER, _REGULAR_USER, FakeTribesStore, _make_fake_check_document_exists, _make_fake_update_document

_test_app = FastAPI()
_test_app.include_router(router, prefix="/api/features/tribes-projects")

FEATURE = "../../../../features/features/tribes_projects/tribes/update_tribe.feature"


@scenario(FEATURE, "PUT /tribes/0010 with valid body as admin — the tribe is updated")
def test_update_admin():
    pass


@scenario(FEATURE, "PUT /tribes/0010 as a viewer — 403 error and the tribe is not modified")
...
def admin_client(tribes_store: FakeTribesStore):
    ...
    with (
        ...
        patch("app.features.tribes_projects.tribes.router.update_document",
              new=AsyncMock(side_effect=_make_fake_update_document(tribes_store))),
    ):
