...
from tests.conftest import _ADMIN_USER, _REGULAR_USER, FakeTribesStore, _make_fake_create_document

_test_app = FastAPI()
_test_app.include_router(router, prefix="/api/features/tribes-projects")

FEATURE = "../../../../features/features/tribes_projects/tribes/create_tribe.feature"


@scenario(FEATURE, "POST /tribes/ with valid body as admin — the tribe is created")
def test_create_admin():
    pass


@scenario(FEATURE, "POST /tribes/ with missing name — 422 error and the database is not modified")
def test_create_missing_field():
    pass


@scenario(FEATURE, "POST /tribes/ as a viewer — 403 error and the database is not modified")
...
def admin_client(tribes_store: FakeTribesStore):
    ...
    with (
        ...
        patch("app.features.tribes_projects.tribes.router.check_unique_field",
              new=AsyncMock(return_value=None)),
        patch("app.platform.core.utils.validators.EntityValidator.validate_references",
              new=AsyncMock(return_value=None)),
        patch("app.features.tribes_projects.tribes.router.create_document",
              new=AsyncMock(side_effect=_make_fake_create_document(tribes_store, "tribes"))),
    ):
