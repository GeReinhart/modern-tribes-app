...
from tests.conftest import _ADMIN_USER, _REGULAR_USER, FakeProjectsStore, _make_fake_delete_document

_test_app = FastAPI()
_test_app.include_router(router, prefix="/api/features/tribes-projects")

FEATURE = "../../../../features/features/tribes_projects/projects/archive_project.feature"


@scenario(FEATURE, "DELETE /projects/0010 as admin — project is archived")
...
def admin_client(projects_store: FakeProjectsStore):
    ...
    with (
        ...
        patch("app.features.tribes_projects.projects.router.delete_document",
              new=AsyncMock(side_effect=_make_fake_delete_document(projects_store))),
    ):
