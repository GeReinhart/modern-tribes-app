@wip
Feature: Mark a grocery list item as picked up
  As a project member
  I want to check off an item once it's in my cart
  ...
  Scenario: PATCH /groceries-list-items/4001 as a project member — the item is marked picked up
    ...
    And the groceries_list_items table contains:
      ...
      | 4001 | 0201               | 3001               | 2.00      | true      | active |

  Scenario: PATCH /groceries-list-items/4001 to update the quantity — the quantity is updated
    Given I am authenticated as a regular user: user.id 0002
    And the positions table contains:
      | id   | tribe_id | person_id | position | status |
      | 1001 | 0010     | 0030      | member   | active |
  ...
  Scenario: PATCH /groceries-list-items/4001 as a project guest — 403 error and the item is not modified
    ...
    And the groceries_list_items table contains:
      | id   | groceries_list_id | groceries_item_id | quantity | picked_up | status |
      | 4001 | 0201               | 3001               | 2.00      | false     | active |

  @error_case
  Scenario: PATCH /groceries-list-items/9999 on a non-existent item — 404 error
    Given I am authenticated as a regular user: user.id 0002
    And the positions table contains:
      | id   | tribe_id | person_id | position | status |
      | 1001 | 0010     | 0030      | member   | active |
    When I PATCH /api/features/tasks/groceries-list-items/9999 with body:
      """
      {"picked_up": true}
      """
