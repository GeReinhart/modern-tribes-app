Feature: Search
  ...
  Scenario: GET /search/?q=Alice as viewer — search results are returned
    Given I am authenticated as a regular user: user.id 0002
    When I GET /api/platform/functions/search/?q=Alice
    Then the response status code is 200

  @error_case
  Scenario: GET /search/?q=Alice as a user with no app access — 403 error
