...
async def insert_feature_label(pool, feature_instance_id: str, name: str, color: str, user_id: str) -> dict:
    async with pool.acquire() as conn:
        ...
        row = await conn.fetchrow(
            ...
            name, color, position, UUID(feature_instance_id), UUID(user_id),
        )
...
async def update_feature_label(
    ...
    async with pool.acquire() as conn:
        if name is not None:
            await conn.execute("UPDATE labels SET name = $1, updated_by = $2 WHERE id = $3", name, UUID(user_id), UUID(label_id))
        if color is not None:
            await conn.execute("UPDATE labels SET color = $1, updated_by = $2 WHERE id = $3", color, UUID(user_id), UUID(label_id))
        row = await conn.fetchrow("SELECT id, name, color, position FROM labels WHERE id = $1", UUID(label_id))
    return {"id": str(row["id"]), "name": row["name"], "color": row["color"], "position": row["position"]} if row else None
...
async def add_entity_label(pool, entity_id: str, entity_type: str, label_id: str) -> None:
    async with pool.acquire() as conn:
        await conn.execute(
            ...
            UUID(label_id), entity_type, UUID(entity_id),
        )
...
async def remove_entity_label(pool, entity_id: str, entity_type: str, label_id: str) -> None:
    async with pool.acquire() as conn:
        await conn.execute(
            ...
            entity_type, UUID(entity_id), UUID(label_id),
        )
...
async def search_feature_labels(pool, name: str, limit: int = 20) -> list[dict]:
    ...
    return [{"id": str(r["id"]), "name": r["name"], "color": r["color"], "feature_instance_id": str(r["feature_instance_id"])} for r in rows]
