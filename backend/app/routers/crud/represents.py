...
from ...models.crud.represents import Represents, RepresentsCreate, RepresentsUpdate
...
from ...routers.auth.authorization import require_permission_decorator, require_any_permission_decorator
...
async def get_represents_by_user(user_id: str, current_user: dict = Depends(get_current_user)):
    ...
    return await get_all_documents(pool, TABLE, filter_query="user_id = $1", params=[UUID(user_id)], any_status=True)
...
@require_permission_decorator(PermissionEnum.ADMIN)
async def update_represents(represents_id: str, represents: RepresentsUpdate, current_user: dict = Depends(get_current_user)):
    pool = get_database()
    validator = EntityValidator(pool)
