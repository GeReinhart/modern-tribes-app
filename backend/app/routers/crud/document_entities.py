...
from ...models.crud.document_entities import DocumentEntity, DocumentEntityCreate, DocumentEntityUpdate
...
@require_permission_decorator(PermissionEnum.ADMIN)
async def create_document_entity(document_entity: DocumentEntityCreate,current_user: dict = Depends(get_current_user)):
    ...
    references = [
        {
            'table': 'documents',
            'id': document_entity.document_id,
            'name': 'Document'
        }
    ]
    ...
    if document_entity.related_document_id:
        references.append({
            'table': 'documents',
            'id': document_entity.related_document_id,
...
@require_permission_decorator(PermissionEnum.ADMIN)
async def update_document_entity(document_entity_id: str, document_entity: DocumentEntityUpdate,current_user: dict = Depends(get_current_user)):
