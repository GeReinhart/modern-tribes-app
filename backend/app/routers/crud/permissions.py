...
from ...models.crud.permissions import Permission, PermissionCreate, PermissionUpdate
...
@require_permission_decorator(PermissionEnum.ADMIN)
async def update_permission(permission_id: str, permission: PermissionUpdate,current_user: dict = Depends(get_current_user)):
...
async def get_permission_roles(permission_id: str,current_user: dict = Depends(get_current_user)):
    ...
    roles = await get_all_documents(
        pool,
        "role_permissions",
        filter_query="permission_id = $1",
        params=[UUID(permission_id)]
    )
