...
@require_permission_decorator(PermissionEnum.ADMIN)
async def add_mail_recipient(mail_id: str, recipient: MailToCreate, current_user: dict = Depends(get_current_user)):
    pool = get_database()
    await check_document_exists(pool, MAIL_TABLE, mail_id, "Mail")
...
@require_permission_decorator(PermissionEnum.ADMIN)
async def remove_mail_recipient(mail_id: str, mail_to_id: str, current_user: dict = Depends(get_current_user)):
    pool = get_database()
