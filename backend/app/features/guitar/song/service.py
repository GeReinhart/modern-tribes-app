...
from app.features.guitar.song.layout.service import collect_song_chords_union, copy_layout_from, get_layout, seed_default_layout
...
async def _create_song_from_copy(pool, project_id: str, data: GuitarSongCreate, user: dict) -> GuitarSongResponse:
    ...
    videos, labels and layout (including every "Lyrics & Chords" block's own content) -- except
    title/author, which stay whatever the caller provided (unlike duplicate_song's auto
    '<title> - COPIE')."""
...
async def duplicate_song(pool, song_id: str, user: dict) -> GuitarSongDetailResponse:
    """Copy a whole song -- description, chords, videos, labels and layout -- into a
