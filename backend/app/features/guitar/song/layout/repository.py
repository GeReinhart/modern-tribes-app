...
_COLUMN_FIELDS = (
    "id::text, row_id::text, position, width_eighths, align, "
...
async def _fetch_blocks_by_column(pool, song_id: str) -> dict[str, list[dict]]:
    ...
    for row in rows:
        grouped[row["column_id"]].append(dict(row))
    return grouped
    ...
    for position, column in enumerate(columns, start=1):
        column_row = await conn.fetchrow(
            """INSERT INTO guitar_songs_layout_columns (
                   row_id, song_id, position, width_eighths, align,
            ...
            UUID(row_id), UUID(song_id), position, column["width_eighths"], column["align"],
