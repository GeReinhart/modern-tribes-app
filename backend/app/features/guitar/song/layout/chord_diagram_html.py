...
_SIZE_SCALE = {"xxs": 0.35, "xs": 0.45, "s": 0.55, "m": 0.7, "l": 0.85, "xl": 1.0, "xxl": 1.2}
# Below this, a "full" (labeled) marker's own interval text no longer fits its circle at all --
# mirrors the frontend's ChordDiagram.tsx MIN_LABELED_SCALE exactly. "simple" markers are plain
# dots with no text, so they have no such floor.
_MIN_LABELED_SCALE = 0.55


def _effective_scale(diagram_size: str, diagram_style: str) -> float:
    scale = _SIZE_SCALE.get(diagram_size, 1.0)
    return scale if diagram_style == "simple" else max(scale, _MIN_LABELED_SCALE)
...
def diagram_pixel_width(diagram_size: str = "m", diagram_style: str = "full", extra_scale: float = 1.0) -> float:
    ...
    scale = _effective_scale(diagram_size, diagram_style) * extra_scale
...
def render_chord_diagram_html(
    frets: list, root_note: str, diagram_style: str = "full", diagram_size: str = "m", min_window_size: int = 0,
