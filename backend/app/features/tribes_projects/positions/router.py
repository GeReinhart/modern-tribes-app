...
from app.features.tribes_projects.positions.models import Position, PositionCreate, PositionUpdate
...
async def create_position(position: PositionCreate, current_user: dict = Depends(get_current_user)):
    ...
    if position.tribe_id:
        references.append({"table": "tribes", "id": position.tribe_id, "name": "Tribe"})
    if position.person_id:
        references.append({"table": "persons", "id": position.person_id, "name": "Person"})
