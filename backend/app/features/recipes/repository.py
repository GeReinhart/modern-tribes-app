...
async def insert_ingredient(
    ...
    custom_unit: Optional[str], quantity: float, user_id: str,
) -> dict:
    async with pool.acquire() as conn:
        ...
        row = await conn.fetchrow(
            """INSERT INTO recipe_ingredients
                   (recipe_id, groceries_item_id, custom_name, custom_unit, quantity, position, created_by, updated_by)
               VALUES ($1, $2, $3, $4, $5, $6, $7, $7) RETURNING *""",
            UUID(recipe_id), UUID(groceries_item_id) if groceries_item_id else None,
            custom_name, custom_unit, quantity, position, UUID(user_id),
        )
    return dict(row)
...
async def update_ingredient(
    pool, ingredient_id: str, quantity: Optional[float], position: Optional[int], user_id: str,
) -> dict:
