...
async def add_bookmark(user_id: str, data: UserBookmarkCreate, pool, current_user: dict) -> UserBookmarkItem:
    row = await user_bookmarks_repository.add_bookmark(
        pool, user_id, data.page_path, data.page_title,
        current_user['id'], data.description, data.color_text, data.color_background,
    )
    return UserBookmarkItem(**row)


async def update_bookmark(
    user_id: str, bookmark_id: str, data: UserBookmarkUpdate, pool, current_user: dict,
) -> UserBookmarkItem:
    row = await user_bookmarks_repository.update_bookmark(
        pool, user_id, bookmark_id, data.page_title,
        data.description, data.color_text, data.color_background, current_user['id'],
    )
    if row is None:
