...
async def get_session(pool, user_id: str, session_id: str) -> dict | None:
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT * FROM user_sessions WHERE user_id = $1 AND session_id = $2", UUID(user_id), session_id
        )
    return row_to_dict(row) if row else None
...
async def delete_session(pool, user_id: str, session_id: str) -> None:
    async with pool.acquire() as conn:
        await conn.execute(
            "DELETE FROM user_sessions WHERE user_id = $1 AND session_id = $2", UUID(user_id), session_id
        )
...
async def get_session_by_refresh_token_hash(pool, token_hash: str) -> dict | None:
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT * FROM user_sessions WHERE refresh_token_hash = $1", token_hash)
    return row_to_dict(row) if row else None
