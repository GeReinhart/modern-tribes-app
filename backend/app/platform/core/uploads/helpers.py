...
async def get_document_with_attachments(pool, document_id: str) -> dict:
    ...
    async with pool.acquire() as conn:
        document_row = await conn.fetchrow("SELECT * FROM documents WHERE id = $1", UUID(document_id))
...
async def create_document_with_attachments(
    pool, content_html: str, attachments: List[AttachmentFile] = None, user_id: str = None
) -> dict:
...
async def update_document_attachments(
    pool, document_id: str, attachments: List[AttachmentFile], user_id: str = None
) -> None:
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute("DELETE FROM document_attachments WHERE document_id = $1", UUID(document_id))
