...
import { useUserWithRolesAndPermissions, useUserMutations } from '@/hooks/useUsers';
import { useRepresentsByUserId, useRepresentsMutations } from '@/hooks/useRepresents';
...
const UserEditPageContent: React.FC = () => {
    ...
    const { user, loading: userLoading } = useUserWithRolesAndPermissions(userId ?? null);
    const { represents, loading: representsLoading } = useRepresentsByUserId(userId ?? null);
    ...
    const handleSubmit = async (_data: UserCreate | UserUpdate, representPersonIds: string[]) => {
        if (!user) return;
        setSubmitError(null);
        try {
            ...
            const toAdd = representPersonIds.filter(id => !existingIds.includes(id));
            const toRemove = represents.filter(r => !representPersonIds.includes(r.person_id));
            await Promise.all([
                ...toAdd.map(pid => createRepresents({ user_id: user.id, person_id: pid, status: 'active' })),
    ...
    if (userLoading || representsLoading) return (
...
export const UserEditPage: React.FC = () => (
    <ThemeProvider defaultTheme={adminMainThemeId}><UserEditPageContent /></ThemeProvider>
);
