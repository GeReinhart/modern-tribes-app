...
const RolesTab: React.FC = () => {
    ...
    const filteredRoles = useMemo(() => {
        ...
        return roles.filter(r =>
            r.name.toLowerCase().includes(term) || (r.description && r.description.toLowerCase().includes(term))
        );
    }, [roles, crud.filter]);
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            render: (r: RoleWithPermissions) => <RolePermissionsBadges permissions={r.permissions} maxDisplay={3} />,
        },
        {
            key: 'created_at',
            header: t('common.created'),
            render: (r: RoleWithPermissions) => new Date(r.created_at).toLocaleDateString(),
        },
        {
            key: 'status',
            header: t('monitoring.status'),
            render: (r: RoleWithPermissions) => <StatusBadge status={r.status ?? 'active'} />,
        },
        {
            key: 'actions',
...
const PermissionsTab: React.FC = () => {
    ...
    const crud = useCrudPage<Permission, PermissionCreate, PermissionUpdate>(
        refetch, createPermission, updatePermission, deletePermission,
    );
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            key: 'status',
            header: t('monitoring.status'),
            render: (p: Permission) => <StatusBadge status={p.status ?? 'active'} />,
        },
        {
            key: 'actions',
            header: t('common.actions'),
        ...
        <AppLayout breadcrumbs={breadcrumbs} headerActions={headerActions}>
            ...
            <ThemedCard>
                ...
                <div style={{ marginTop: '16px' }}>
                    <div style={{ display: activeTab === 'roles' ? 'block' : 'none' }}><RolesTab /></div>
                    <div style={{ display: activeTab === 'permissions' ? 'block' : 'none' }}><PermissionsTab /></div>
                </div>
            </ThemedCard>
        </AppLayout>
    );
...
export const AuthorizationPage: React.FC = () => (
    <ThemeProvider defaultTheme={adminMainThemeId}><AuthorizationPageContent /></ThemeProvider>
);
