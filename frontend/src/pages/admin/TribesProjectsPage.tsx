...
import { ThemedModal, ThemedModalBody } from '@/components/common/layout/ThemedModal';
...
const TribesTab: React.FC = () => {
    ...
    const crud = useCrudPage<Tribe, TribeCreate, TribeUpdate>(refetch, async () => undefined, async () => undefined, deleteTribe);
    ...
    const tribePositions = useMemo(() => {
        ...
        for (const pos of positions) {
            ...
            map.get(pos.tribe_id)!.push({ personId: pos.person_id, position: pos.position });
        }
        return map;
    }, [positions]);
    ...
    const [membersPopupTribeId, setMembersPopupTribeId] = useState<string | null>(null);
    ...
    const filteredTribes = useMemo(() => {
        ...
        if (personFilter) {
            list = list.filter(tr => tribePositions.get(tr.id)?.some(e => e.personId === personFilter));
        }
        return list;
    ...
    const popupTribe = useMemo(() => membersPopupTribeId ? tribes.find(tr => tr.id === membersPopupTribeId) : null, [membersPopupTribeId, tribes]);
    const popupMembers = useMemo(() => membersPopupTribeId ? (tribePositions.get(membersPopupTribeId) ?? []) : [], [membersPopupTribeId, tribePositions]);
    ...
    const columns = useMemo(() => [
        ...
        {
            key: 'persons', header: t('admin.columnPersons'),
            render: (tr: Tribe) => {
                const count = tribePositions.get(tr.id)?.length ?? 0;
                return (
                    <button
                        onClick={e => { e.stopPropagation(); setMembersPopupTribeId(tr.id); }}
                        style={{
                            padding: '2px 10px',
                            borderRadius: '12px',
                            border: `1.5px solid ${count > 0 ? theme.colors.primary : theme.colors.border}`,
                            background: count > 0 ? `${theme.colors.primary}18` : 'transparent',
                            color: count > 0 ? theme.colors.primary : theme.colors.secondary,
                            fontSize: '12px',
                            fontWeight: 600,
                            cursor: 'pointer',
                        }}
        ...
        },
        {
            ...
            render: (tr: Tribe) => (
                <span style={{ fontSize: '12px', color: theme.colors.secondary }}>
                    {tr.document_id
                        ? <span style={{ color: theme.colors.primary }}>{t('admin.attached')}</span>
                        : t('admin.none')}
                </span>
            ),
        },
    ...
    ], [t, theme.colors.secondary, theme.colors.primary, theme.colors.border, crud, navigate, tribePositions]);
    ...
    return (
        <>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
                <ThemedButton variant="secondary" onClick={() => navigate('/admin/tribes/new')}>{t('admin.addTribe')}</ThemedButton>
            ...
            >
                <ThemedModalBody>
                    {popupMembers.length === 0 ? (
                        <ThemedText variant="secondary" size="small">{t('admin.noTribeMembers')}</ThemedText>
                    ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {popupMembers.map(entry => {
                                const person = personById.get(entry.personId);
                                return (
                                    <div key={entry.personId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <ThemedText variant="text" size="small">
                                            {person ? `${person.first_name} ${person.last_name}` : entry.personId}
                                        </ThemedText>
                                        <span style={{
                                            padding: '2px 8px',
                                            borderRadius: '10px',
                                            backgroundColor: POSITION_COLORS[entry.position] ?? '#6b7280',
                                            color: 'white',
                                            fontSize: '11px',
                                            fontWeight: 600,
        ...
        <AppLayout breadcrumbs={breadcrumbs} headerActions={headerActions}>
            <ThemedCard>
                ...
                <div style={{ marginTop: '16px' }}>
                    <div style={{ display: activeTab === 'tribes' ? 'block' : 'none' }}><TribesTab /></div>
                    <div style={{ display: activeTab === 'projects' ? 'block' : 'none' }}><ProjectsTab /></div>
                </div>
            </ThemedCard>
        </AppLayout>
    );
...
export const TribesProjectsPage: React.FC = () => (
    <ThemeProvider defaultTheme={adminMainThemeId}><TribesProjectsPageContent /></ThemeProvider>
);
