...
import { AppConfigEntry, AppConfigCreate } from '@/types/app-config.types';
...
const ConfigRow: React.FC<{
    ...
    const handleCancel = () => {
        setState({ value: entry.value, description: entry.description ?? '', saving: false });
        setEditing(false);
    };
    ...
    return (
        <tr>
            <td style={cellStyle}>
                <code style={{ fontSize: 'var(--font-sm)', color: theme.colors.primary }}>{entry.key}</code>
            </td>
            <td style={cellStyle}>
                {editing ? (
                    <input
            ...
            <td style={{ ...cellStyle, whiteSpace: 'nowrap' }}>
                {editing ? (
                    <div style={{ display: 'flex', gap: 'var(--space-xs)' }}>
                        <ThemedButton variant="secondary" onClick={handleSave} disabled={state.saving}>
                            {t('common.save')}
                        </ThemedButton>
                        <ThemedButton variant="ghost" onClick={handleCancel} disabled={state.saving}>
                            {t('common.cancel')}
                        </ThemedButton>
                    </div>
                ) : (
...
const AddConfigForm: React.FC<{ onAdded: (entry: AppConfigEntry) => void; onCancel: () => void }> = ({ onAdded, onCancel }) => {
    const { t } = useTranslation();
    const { theme } = useTheme();
    const [form, setForm] = useState<AppConfigCreate>({ key: '', value: '', description: '' });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            const entry = await appConfigService.create({ ...form, description: form.description || undefined });
            onAdded(entry);
        } catch (err: unknown) {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 2fr auto', gap: 'var(--space-sm)', alignItems: 'end' }}>
                ...
                <div style={{ display: 'flex', gap: 'var(--space-xs)', paddingBottom: 0 }}>
                    <ThemedButton variant="secondary" type="submit" disabled={saving}>
                        {t('common.create')}
                    </ThemedButton>
                    <ThemedButton variant="ghost" type="button" onClick={onCancel} disabled={saving}>
                        {t('common.cancel')}
                    </ThemedButton>
                </div>
            </div>
        ...
        <AppLayout breadcrumbs={breadcrumbs} headerActions={headerActions}>
            ...
            <ThemedCard>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-md)' }}>
                    <ThemedText size="small">{t('admin.foundResults', { count: entries.length })}</ThemedText>
                    {!showAddForm && (
                        <ThemedButton variant="secondary" onClick={() => setShowAddForm(true)}>
                            {t('admin.config.addEntry')}
                        </ThemedButton>
                    )}
                </div>
