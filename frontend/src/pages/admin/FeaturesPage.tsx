...
import { ProjectFeatureInstance, FeatureTypeInfo } from '@/types/project-features.types';
...
function FeaturesPageContent() {
    ...
    const [createForm, setCreateForm] = useState<CreateFormState>({ feature_type: '', name: '' });
    const [creating, setCreating] = useState(false);

    const [confirm, setConfirm] = useState<{ id: string; name: string; nextStatus: 'active' | 'archived' } | null>(null);
    ...
    const handleProjectChange = (value: string) => {
        ...
        if (value) {
            loadFeatures(value);
            apiService.get<ProjectTribeEntry[]>(`/query/projects/${value}/tribes`)
    ...
    const handleStatusChange = async () => {
        ...
        setActioning(true);
        try {
            const updated = await projectFeaturesService.update(selectedProjectId, confirm.id, { status: confirm.nextStatus });
            setFeatures(prev => prev.map(f => f.id === confirm.id ? { ...f, status: updated.status } : f));
        } catch (e: any) {
            alert(e.message);
        } finally {
            setActioning(false);
    ...
    const handleCreate = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!selectedProjectId || !createForm.feature_type || !createForm.name.trim()) return;
        setCreating(true);
        try {
    ...
    const columns = useMemo(() => [
        {
            ...
            render: (f: ProjectFeatureInstance) => (
                <ThemedText variant="primary" size="small">{f.name}</ThemedText>
            ),
        },
        {
            key: 'feature_type',
            ...
            render: (f: ProjectFeatureInstance) => (
                <code style={{ fontSize: 'var(--font-sm)', color: theme.colors.primary }}>{f.feature_type}</code>
            ),
        },
        {
            key: 'status',
            header: t('monitoring.status'),
            render: (f: ProjectFeatureInstance) => <StatusBadge status={f.status} />,
        },
        {
            key: 'actions',
            ...
            render: (f: ProjectFeatureInstance) => (
                ...
                >
                    {f.status === 'active' ? t('tribes.archive') : t('admin.features.restore')}
                </ThemedButton>
            ),
        },
    ], [t, theme]);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs} headerActions={headerActions}>
            <ThemedCard>
                <div style={{ display: 'flex', gap: 'var(--space-sm)', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: 220 }}>
                        <ThemedText variant="secondary" size="small" style={{ marginBottom: 4 }}>
                            {t('common.project')}
                        </ThemedText>
                        {projectsLoading ? (
                    ...
                    {selectedProjectId && (
                        <ThemedButton variant="primary" onClick={() => setShowCreateForm(s => !s)}>
            ...
            {showCreateForm && selectedProjectId && (
                <ThemedCard>
                    <form onSubmit={handleCreate}>
                        <ThemedText variant="primary" size="small" style={{ marginBottom: 'var(--space-sm)', fontWeight: 600 }}>
                            {t('features.addFeature')}
                        </ThemedText>
                        <div style={{ display: 'flex', gap: 'var(--space-sm)', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                            <div>
                                ...
                                <ThemedSelect
                                    ...
                                    onChange={v => setCreateForm(f => ({ ...f, feature_type: v }))}
                            ...
                            <ThemedButton
                                variant="primary"
                                type="submit"
                                disabled={creating || !createForm.feature_type || !createForm.name.trim()}
                            >
                                {t('common.create')}
                            </ThemedButton>
                            <ThemedButton variant="ghost" type="button" onClick={() => setShowCreateForm(false)}>
                                {t('common.cancel')}
                            </ThemedButton>
                        </div>
                    </form>
            ...
            {selectedProjectId && (
                <ThemedCard>
                    {featuresError && (
                        <ThemedText variant="danger" size="small" style={{ marginBottom: 'var(--space-sm)' }}>
                            {featuresError}
                        </ThemedText>
                    )}
                    {featuresLoading ? (
            ...
            {!selectedProjectId && !projectsLoading && (
                <ThemedCard>
                    <ThemedText variant="secondary">{t('admin.features.selectProject')}</ThemedText>
                </ThemedCard>
            )}
            ...
            <ThemedConfirmDialog
                ...
                title={confirm?.nextStatus === 'archived' ? t('features.archiveTitle') : t('admin.features.restoreTitle')}
                message={
                    confirm?.nextStatus === 'archived'
                        ? t('features.archiveMessage', { name: confirm?.name ?? '' })
                        : t('admin.features.restoreMessage', { name: confirm?.name ?? '' })
                }
                confirmText={confirm?.nextStatus === 'archived' ? t('tribes.archive') : t('admin.features.restore')}
