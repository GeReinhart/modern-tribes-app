...
import { useTribe, useTribeProjects, useTribeMutations } from '@/hooks/useTribes';
...
import { TribeCreate, TribeUpdate, TribeProjectInput, TribeProjectRelation } from '@/types/tribe.types';
...
const ProjectRelationsSection: React.FC<ProjectSectionProps> = ({ projects, projectRows, onChange }) => {
    ...
    const filtered = useMemo(() => {
        let list = projects;
        if (showOnlySelected) list = list.filter(p => projectRows.some(r => r.project_id === p.id));
        if (filter.trim()) {
    ...
    const toggle = useCallback((projectId: string) => {
        ...
        if (exists) onChange(projectRows.filter(r => r.project_id !== projectId));
        else onChange([...projectRows, { project_id: projectId, relation: 'member' }]);
    }, [projectRows, onChange]);

    const updateRelation = useCallback((projectId: string, relation: string) => {
        onChange(projectRows.map(r => r.project_id === projectId ? { ...r, relation: relation as TribeProjectRelation } : r));
    }, [projectRows, onChange]);

    return (
        <div>
            <div style={{ fontSize: '13px', fontWeight: 600, color: theme.colors.primary, marginBottom: '8px' }}>
                Project relations
            </div>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '6px' }}>
                <div style={{ flex: 1 }}>
                    <ThemedInput label="" value={filter} onChange={e => setFilter(e.target.value)} placeholder="Filter projects..." variant="primary" />
                </div>
                <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: theme.colors.text, cursor: 'pointer', whiteSpace: 'nowrap', userSelect: 'none' }}>
                    <input
                        type="checkbox"
                        checked={showOnlySelected}
                        onChange={e => setShowOnlySelected(e.target.checked)}
                        style={{ width: '14px', height: '14px', cursor: 'pointer', accentColor: theme.colors.primary }}
                    />
            ...
            <div style={{ maxHeight: '220px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '4px', paddingRight: '2px' }}>
                {filtered.length === 0 && (
                    <div style={{ fontSize: '13px', color: theme.colors.secondary, padding: '8px 0' }}>
                ...
                {filtered.map(project => {
                    ...
                    const color = row ? RELATION_COLORS[row.relation] : theme.colors.primary;
                    return (
                        <div key={project.id} onClick={() => toggle(project.id)} style={{
                            display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 12px',
                            borderRadius: '8px',
                            border: `2px solid ${isSelected ? color : theme.colors.primary + '25'}`,
                            background: isSelected ? `${color}12` : `${theme.colors.primary}05`,
                            cursor: 'pointer', transition: 'all 0.15s',
                        }}>
                            <input type="checkbox" checked={isSelected} readOnly style={{ width: '15px', height: '15px', cursor: 'pointer', accentColor: color, flexShrink: 0 }} />
                            <span style={{ fontSize: '13px', color: theme.colors.text, flex: 1 }}>{project.name}</span>
                            {isSelected && (
                                <div onClick={e => e.stopPropagation()}>
                                    <select value={row!.relation} onChange={e => updateRelation(project.id, e.target.value)} style={{
                                        padding: '2px 8px', borderRadius: '6px', border: `1.5px solid ${color}`,
                                        background: `${color}15`, color, fontSize: '12px', fontWeight: 600, cursor: 'pointer', outline: 'none',
                                    }}>
...
const PersonPositionsSection: React.FC<PersonSectionProps> = ({ persons, personPositions, onChange }) => {
    const { theme } = useTheme();
    const [filter, setFilter] = useState('');
    const [showOnlySelected, setShowOnlySelected] = useState(true);
    ...
    const filtered = useMemo(() => {
        let list = persons;
        if (showOnlySelected) list = list.filter(p => personPositions.some(e => e.person_id === p.id));
        if (filter.trim()) {
            const term = filter.toLowerCase();
            list = list.filter(p => `${p.first_name} ${p.last_name}`.toLowerCase().includes(term));
        }
        return list;
    ...
    const toggle = useCallback((personId: string) => {
        ...
        if (exists) onChange(personPositions.filter(e => e.person_id !== personId));
        else onChange([...personPositions, { person_id: personId, position: 'member' }]);
    }, [personPositions, onChange]);

    const updatePosition = useCallback((personId: string, position: string) => {
        onChange(personPositions.map(e => e.person_id === personId ? { ...e, position: position as PositionEnum } : e));
    }, [personPositions, onChange]);
    ...
    return (
        <div>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '6px' }}>
                <div style={{ flex: 1 }}>
                    <ThemedInput label="" value={filter} onChange={e => setFilter(e.target.value)} placeholder="Filter persons..." variant="primary" />
                </div>
                <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', color: theme.colors.text, cursor: 'pointer', whiteSpace: 'nowrap', userSelect: 'none' }}>
                    <input
                        type="checkbox"
            ...
            <div style={{ maxHeight: '240px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '4px', paddingRight: '2px' }}>
                {filtered.length === 0 && (
                    <div style={{ fontSize: '13px', color: theme.colors.secondary, padding: '8px 0' }}>
                ...
                {filtered.map(person => {
                    ...
                    const color = entry ? POSITION_COLORS[entry.position] : theme.colors.primary;
                    return (
                        <div key={person.id} onClick={() => toggle(person.id)} style={{
                            ...
                            <input type="checkbox" checked={isSelected} readOnly style={{ width: '15px', height: '15px', cursor: 'pointer', accentColor: color, flexShrink: 0 }} />
                            <span style={{ fontSize: '13px', color: theme.colors.text, flex: 1 }}>
                                {person.first_name} {person.last_name}
                            </span>
                            {isSelected && (
                                <div onClick={e => e.stopPropagation()}>
                                    <select value={entry!.position} onChange={e => updatePosition(person.id, e.target.value)} style={{
                                        padding: '2px 8px', borderRadius: '6px', border: `1.5px solid ${color}`,
                                        background: `${color}15`, color, fontSize: '12px', fontWeight: 600, cursor: 'pointer', outline: 'none',
                                    }}>
...
const TribeEditPageContent: React.FC = () => {
    ...
    const { tribeProjects, hasFetched: projectsFetched } = useTribeProjects(tribeId ?? null);
    const { positions: existingPositions, hasFetched: positionsFetched } = usePositionsByTribe(tribeId ?? null);
    const { projects } = useProjects();
    const { persons } = usePersons();
    const { documents } = useDocuments();
    const { createTribe, updateTribe } = useTribeMutations();

    const [formData, setFormData] = useState<{ name: string; document_id: string | null }>({ name: '', document_id: null });
    ...
    useEffect(() => {
        if (!projectsInitialized.current && projectsFetched) {
            setProjectRows(tribeProjects.map(tp => ({ project_id: tp.project_id, relation: tp.relation as TribeProjectRelation })));
    ...
    useEffect(() => {
        if (!positionsInitialized.current && positionsFetched) {
            setPersonPositions(existingPositions.map(p => ({
    ...
    const syncPositions = useCallback(async (savedTribeId: string) => {
        if (isCreate) {
            for (const entry of personPositions) {
                await positionService.create({ tribe_id: savedTribeId, person_id: entry.person_id, position: entry.position });
            }
            return;
        }
        const existingMap = new Map(existingPositions.map(p => [p.person_id!, p]));
        ...
        for (const entry of personPositions) {
            ...
            if (!existing) {
                ...
                await positionService.update(existing.id, { position: entry.position });
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isCreate) {
                ...
                const updated = await updateTribe(tribeId!, { ...formData, status } as TribeUpdate);
    ...
    const isLoading = !isCreate && (tribeLoading || !projectsFetched || !positionsFetched);
    ...
    const documentOptions = [
        { value: '', label: 'None' },
        ...documents.map(d => ({ value: d.id, label: d.content_summary ? `${d.content_summary} (${d.id.slice(0, 8)})` : d.id.slice(0, 8) })),
    ];
        ...
        <AppLayout breadcrumbs={breadcrumbs} headerActions={headerActions}>
            <form onSubmit={handleSubmit}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    <ThemedCard>
                        <ThemedText variant="primary" size="medium" as="h3" style={{ marginBottom: '16px' }}>
                            {t('admin.tribeInfo')}
                        </ThemedText>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            ...
                            <ThemedSelect
                                ...
                                onChange={v => setFormData(p => ({ ...p, document_id: v || null }))}
                    ...
                    <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                        <ThemedButton type="button" variant="ghost" onClick={() => navigate('/admin/tribes')}>
                            {t('common.cancel')}
                        </ThemedButton>
...
export const TribeEditPage: React.FC = () => (
    <ThemeProvider defaultTheme={adminMainThemeId}><TribeEditPageContent /></ThemeProvider>
);
