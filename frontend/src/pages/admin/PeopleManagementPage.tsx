...
import { useRepresents } from '@/hooks/useRepresents';
...
import { ThemedModal, ThemedModalBody, ThemedModalFooter } from '@/components/common/layout/ThemedModal';
...
const UsersTab: React.FC = () => {
    ...
    const [magicLinkModal, setMagicLinkModal] = useState<{ isOpen: boolean; url: string; email: string }>({
        isOpen: false, url: '', email: '',
    });
    ...
    const handleSendMagicLink = useCallback(async (user: UserWithRolesAndPermissions) => {
        ...
        try {
            ...
            setSendFeedback({ success: true, message: t('admin.people.magicLinkSent', { email: user.email }) });
            setTimeout(() => setSendFeedback(null), 4000);
        } catch (e) {
            const msg = e instanceof Error ? e.message : t('admin.people.magicLinkError');
    ...
    const handleGenerateMagicLink = useCallback(async (user: UserWithRolesAndPermissions) => {
        setMagicLinkAction({ userId: user.id, type: 'generate' });
        try {
            const result = await userService.generateMagicLink(user.id);
            setMagicLinkModal({ isOpen: true, url: result.magic_link, email: result.email });
        } catch (e) {
            const msg = e instanceof Error ? e.message : t('admin.people.magicLinkError');
            setSendFeedback({ success: false, message: msg });
            setTimeout(() => setSendFeedback(null), 6000);
    ...
    const getPersonName = useCallback((personId: string): string | null => {
        const person = persons.find(p => p.id === personId);
        return person ? `${person.first_name} ${person.last_name}` : null;
    }, [persons]);

    const getRepresentedPersonIds = useCallback((userId: string): string[] =>
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            render: (u: UserWithRolesAndPermissions) => <UserRolesBadges roles={u.roles} maxDisplay={3} />,
        },
        {
            key: 'persons',
            ...
            render: (u: UserWithRolesAndPermissions) => {
                ...
                if (!directName && representedIds.length === 0) return <span style={{ color: theme.colors.secondary, fontSize: '12px' }}>—</span>;
                return (
        ...
        },
        {
            ...
            render: (u: UserWithRolesAndPermissions) => new Date(u.created_at).toLocaleDateString(),
        },
        {
            key: 'status',
            header: t('monitoring.status'),
            render: (u: UserWithRolesAndPermissions) => <StatusBadge status={u.status ?? 'active'} />,
        },
        {
            key: 'actions',
    ...
    ], [t, theme.colors.secondary, theme.colors.primary, theme.colors.accent, crud, magicLinkAction, handleSendMagicLink, handleGenerateMagicLink, getPersonName, getRepresentedPersonIds]);

    return (
        <>
            {sendFeedback && (
                <ThemedCard variant={sendFeedback.success ? 'success' : 'danger'}>
                    <ThemedText variant={sendFeedback.success ? 'success' : 'danger'} size="small">
            ...
            />
            {loading
                ? <div style={{ display: 'flex', justifyContent: 'center', padding: '32px' }}><ThemedLoadingSpinner size="sm" /></div>
                : (
                    <>
            ...
            >
                <ThemedModalBody>
                    <ThemedText size="small" variant="secondary">{t('admin.people.magicLinkWarning')}</ThemedText>
                    <div style={{
                ...
                <ThemedModalFooter>
                    <ThemedButton variant="secondary" onClick={closeMagicLinkModal}>
                        {t('common.close')}
                    </ThemedButton>
...
const PersonsTab: React.FC = () => {
    ...
    const filteredPersons = useMemo(() => {
        ...
        return persons.filter(p =>
            [p.first_name, p.last_name, `${p.first_name} ${p.last_name}`, p.gender, getGenderLabel(p.gender)]
...
const PeopleManagementPageContent: React.FC = () => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs} headerActions={headerActions}>
            ...
            <ThemedCard>
                <ThemedTabs tabs={tabs} activeTab={activeTab} onTabChange={k => setActiveTab(k as TabKey)} />
                <div style={{ marginTop: '16px' }}>
                    <div style={{ display: activeTab === 'users' ? 'block' : 'none' }}><UsersTab /></div>
                    <div style={{ display: activeTab === 'persons' ? 'block' : 'none' }}><PersonsTab /></div>
                </div>
            </ThemedCard>
        </AppLayout>
    );
...
export const PeopleManagementPage: React.FC = () => (
    <ThemeProvider defaultTheme={adminMainThemeId}><PeopleManagementPageContent /></ThemeProvider>
);
