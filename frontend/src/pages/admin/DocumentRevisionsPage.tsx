...
const DocumentRevisionsPageContent: React.FC = () => {
    ...
    const { documentId, date } = useParams<{ documentId: string; date?: string }>();
    ...
    const selectedRevision = useMemo<DocumentRevision | null>(() => {
        ...
        return (
            ...
            revisions.reduce((closest, r) => {
                ...
                const bestDiff = Math.abs(new Date(closest.updated_at).getTime() - targetMs);
    ...
    const navigateTo = (revision: DocumentRevision) => {
        navigate(`/admin/monitoring/documents/${documentId}/updated_at/${formatDateForUrl(revision.updated_at)}`);
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs} headerActions={headerActions}>
            ...
            {!error && revisions.length === 0 && (
                <ThemedCard>
                    <ThemedText size="small" variant="secondary">{t('monitoring.revisions.none')}</ThemedText>
                </ThemedCard>
            )}
            ...
            {revisions.length > 0 && (
                <div style={{ display: 'flex', gap: 'var(--space-md)', alignItems: 'flex-start' }}>
                    ...
                    <ThemedCard>
                        <ThemedText size="small" style={{ fontWeight: 600, marginBottom: 'var(--space-sm)' }}>
                                ...
                                return (
                                    ...
                                    >
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-xs)' }}>
                                            <span style={{ fontSize: 'var(--font-sm)', fontWeight: isSelected ? 600 : 400 }}>
                                            ...
                                            {r.is_current && (
                                                <span style={{
                                                    ...
                                                    padding: '1px 5px',
                                                    borderRadius: '4px',
                                                    background: isSelected ? 'rgba(255,255,255,0.25)' : theme.colors.success,
                                        ...
                                        {r.updated_by && (
                                            <div style={{ fontSize: 'var(--font-xs, 11px)', opacity: 0.8, marginTop: 2 }}>
                                                {r.updated_by}
                                            </div>
                                        )}
                                    </div>
                                ...
                                <ThemedCard>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-sm)' }}>
                                        <ThemedText size="small" variant="secondary">
                                            ...
                                            {selectedRevision.updated_by && ` — ${selectedRevision.updated_by}`}
                                        </ThemedText>
                                    ...
                                    <div
                                        ...
                                        dangerouslySetInnerHTML={{ __html: selectedRevision.content_html }}
                                    />
                                </ThemedCard>
                            </>
                        ) : (
                            <ThemedCard>
                                <ThemedText size="small" variant="secondary">{t('monitoring.revisions.selectOne')}</ThemedText>
                            </ThemedCard>
                        )}
                    </div>
                </div>
...
export const DocumentRevisionsPage: React.FC = () => (
    <ThemeProvider defaultTheme={adminMainThemeId}><DocumentRevisionsPageContent /></ThemeProvider>
);
