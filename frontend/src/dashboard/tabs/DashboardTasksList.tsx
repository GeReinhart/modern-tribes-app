...
interface Props {
    ...
    onCreateLabel: (task: DashboardTask, data: { feature_instance_id: string; name: string; color: string }) => Promise<TaskLabelInfo | null>;
}
...
const DashboardTasksList: React.FC<Props> = ({
    data, persons, canEdit, onUpdate, onToggleLabel, onCreateLabel,
}) => {
    ...
    if (allTasks.length === 0) {
        return (
            <div style={{ padding: '32px 0', textAlign: 'center', color: theme.colors.secondary, fontSize: 'var(--font-sm)' }}>
