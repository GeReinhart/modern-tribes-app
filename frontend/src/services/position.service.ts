...
class PositionService {
    ...
    async updateByForeignIds(person_id: string, tribe_id: string,data: PositionUpdate): Promise<Position> {
        return apiService.put<Position>(`${this.endpoint}/by/foreign_ids/${person_id}/${tribe_id}`, data);
    ...
    async deleteByForeignIds(person_id: string, tribe_id: string): Promise<void> {
        return apiService.delete<void>(`${this.endpoint}/by/foreign_ids/${person_id}/${tribe_id}`);
        // TODO Implement it on backend
    }
