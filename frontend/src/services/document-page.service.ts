...
class DocumentPageService {
    ...
    list(projectId: string, projectDocumentId: string): Promise<DocumentPage[]> {
        return apiService.get<DocumentPage[]>(this.base(projectId, projectDocumentId));
    }

    create(projectId: string, projectDocumentId: string, data: DocumentPageCreate): Promise<DocumentPage> {
        return apiService.post<DocumentPage>(this.base(projectId, projectDocumentId), data);
    }

    get(projectId: string, projectDocumentId: string, pageId: string): Promise<DocumentPage> {
        return apiService.get<DocumentPage>(`${this.base(projectId, projectDocumentId)}/${pageId}`);
    }

    update(projectId: string, projectDocumentId: string, pageId: string, data: DocumentPageUpdate): Promise<DocumentPage> {
        return apiService.put<DocumentPage>(`${this.base(projectId, projectDocumentId)}/${pageId}`, data);
    }
