...
class NotificationService {
    async searchUsers(q: string): Promise<UserSearchResult[]> {
        return apiService.get<UserSearchResult[]>(`/query/users/search?q=${encodeURIComponent(q)}`);
    }
    ...
    async reportStatus(
        ...
        return apiService.patch<AppNotification>(`/notifications/${notificationId}/status`, {
