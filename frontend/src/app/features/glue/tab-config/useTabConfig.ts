import { TabDefinition } from '@/app/features/glue/url-tab/useUrlTab.ts';

import { useCallback, useEffect, useMemo, useState } from 'react';

import { tabConfigService } from './tabConfig.service.ts';
...
export function useTabConfig(
  ...
  const tabsWithConfig = useMemo(
    ...
    [allTabs.map((t) => `${t.key}:${t.color ?? ''}:${t.icon ?? ''}`).join(','), savedConfigs],
  );
