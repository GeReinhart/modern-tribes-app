...
const SongDetailPageContent: React.FC = () => {
  ...
  const handleEdit = async (data: GuitarSongUpdate) => {
    if (!song) return;
    await guitarSongsService.updateSong(song.id, data);
    await reload();
  };
  ...
  const menuActions = useMemo(() => {
    if (!song) return [];
    return [
      ...(canEdit ? [{ icon: 'pencil' as const, label: t('common.edit'), onClick: () => setEditOpen(true) }] : []),
      ...(isManager
        ? [{ icon: 'trash' as const, label: t('guitarSong.detail.archive'), onClick: () => setArchiveConfirmOpen(true) }]
        : []),
    ];
  }, [song, canEdit, isManager, t]);
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs} menuActions={menuActions} bookmarkSlot={bookmarkSlot}>
      <ThemedSection themeId="main_1">
        {song.author && (
          <div style={{ color: theme.colors.secondary, fontSize: '13px', marginBottom: '12px' }}>{song.author}</div>
        )}
