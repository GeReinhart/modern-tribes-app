...
export const SongColumnMenu: React.FC<SongColumnMenuProps> = ({ rows, row, column, hook, onOpenChange }) => {
  ...
  const handleAddElement = (blockType: LayoutBlockType) =>
    hook.replaceLayoutRow(row.id, layoutMutations.addBlock(row, column.id, blockType));
  const handleAddFreeText = () => hook.replaceLayoutRow(row.id, layoutMutations.addBlock(row, column.id, 'custom'));
  ...
  const handleRemoveColumn = () => hook.replaceLayoutRow(row.id, layoutMutations.removeColumn(row, column.id));
