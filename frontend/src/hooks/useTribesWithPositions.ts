...
import { TribeWithPositionsCreate, TribeWithPositionsUpdate, TribeWithPositionsResponse } from '@/types/app/tribe_with_positions.types.ts';
...
export function useTribeWithPositions(tribeId: string | null) {
    ...
    const fetchTribe = useCallback(async () => {
        ...
        try {
            const data = await execute(() => tribeWithPositionService.getWithPositions(tribeId));
...
export function useTribeWithPositionsMutations() {
    ...
    const updateTribeWithPositions = useCallback(
        async (id: string, data: TribeWithPositionsUpdate) => {
            return execute(() => tribeWithPositionService.updateWithPositions(id, data));
        },
        [execute]
    );
