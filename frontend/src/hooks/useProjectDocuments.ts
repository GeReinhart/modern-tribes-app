...
import { ProjectDocumentSummary, ProjectDocumentLabel, ProjectDocument } from '../types/project-document.types';
...
export function useProjectDocuments(
    ...
    const fetch = useCallback(() => {
        ...
        setLoading(true);
        setError(null);
        projectDocumentService.list(projectId, q, labelId)
...
export function useProjectDocumentLabels(projectId: string | null) {
    ...
    const fetch = useCallback(() => {
        if (!projectId) return;
        setLoading(true);
        projectDocumentService.getLabels(projectId)
            .then(setLabels)
            .finally(() => setLoading(false));
    }, [projectId]);
...
export function useProjectDocument(projectId: string | null, projectDocumentId: string | null) {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError(null);
        projectDocumentService.get(projectId, projectDocumentId)
            .then(setDocument)
            .catch(e => setError(e.message))
            .finally(() => setLoading(false));
