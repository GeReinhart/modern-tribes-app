...
const { useList, useById, useMutations } = createEntityHooks<Permission, PermissionCreate, PermissionUpdate>(permissionService, 'permissions');
...
export function usePermissionMutations() {
    const { create: createPermission, update: updatePermission, remove: deletePermission, ...rest } = useMutations();
