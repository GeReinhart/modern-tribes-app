...
export function useCrudPage<T extends { id: string }, CreateDto, UpdateDto>(
    ...
    const [modalState, setModalState] = useState<ModalState<T>>({ isOpen: false, mode: 'create' });
    const [deleteDialog, setDeleteDialog] = useState<DeleteDialog<T>>({ isOpen: false });
    ...
    const handleDeleteSelected = useCallback(() => {
        if (selectedRows.size > 0) setDeleteDialog({ isOpen: true, isMultiple: true });
    }, [selectedRows]);
    ...
    const handleSubmit = useCallback(async (data: CreateDto | UpdateDto) => {
        try {
    ...
    const openDeleteSingle = useCallback((entity: T) => setDeleteDialog({ isOpen: true, entity }), []);
    const closeDeleteDialog = useCallback(() => setDeleteDialog({ isOpen: false }), []);
