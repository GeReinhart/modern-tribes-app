...
function parseConfig(entries: { key: string; value: string }[]): AppConfigValues {
    const map = Object.fromEntries(entries.map(e => [e.key, e.value]));
    return {
        uploadMaxFiles: parseInt(map['upload.max_files'] ?? '5', 10) || DEFAULT_CONFIG.uploadMaxFiles,
        uploadMaxFileSizeMb: parseInt(map['upload.max_file_size_mb'] ?? '10', 10) || DEFAULT_CONFIG.uploadMaxFileSizeMb,
        editorImageExtensions: map['editor.image_extensions']
            ? map['editor.image_extensions'].split(',').map(s => s.trim()).filter(Boolean)
