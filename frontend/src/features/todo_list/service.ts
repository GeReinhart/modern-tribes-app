...
class TodoListService {
    ...
    async listLabels(featureInstanceId: string): Promise<TodoLabel[]> {
        return apiService.get<TodoLabel[]>(`/features/todo-labels/by-instance/${featureInstanceId}`);
    }
    ...
    async toggleLabel(itemId: string, labelId: string): Promise<string[]> {
        return apiService.post<string[]>(`/features/todo-items/${itemId}/labels/${labelId}`, {});
    }

    async listPersons(featureInstanceId: string): Promise<PersonOption[]> {
        return apiService.get<PersonOption[]>(`/features/todo-items/persons/${featureInstanceId}`);
    }
}
