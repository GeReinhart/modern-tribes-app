...
import { TodoItem, TodoItemCreate, TodoItemUpdate, TodoLabel, TodoLabelCreate, TodoLabelUpdate } from './types';
...
export function useTodoList(featureInstanceId: string | null) {
    ...
    const createItem = useCallback(async (data: TodoItemCreate): Promise<TodoItem | null> => {
        try {
            ...
            return created;
        } catch (e: unknown) { setError(e instanceof Error ? e.message : 'Error'); return null; }
    }, []);
    ...
    const createLabel = useCallback(async (data: TodoLabelCreate): Promise<TodoLabel | null> => {
        try {
    ...
    const updateLabel = useCallback(async (labelId: string, data: TodoLabelUpdate): Promise<void> => {
        try {
    ...
    const deleteLabel = useCallback(async (labelId: string): Promise<void> => {
        try {
            ...
            setItems(prev => prev.map(i => ({ ...i, label_ids: i.label_ids.filter(id => id !== labelId) })));
        } catch (e: unknown) { setError(e instanceof Error ? e.message : 'Error'); }
    }, []);

    const toggleLabel = useCallback(async (itemId: string, labelId: string): Promise<void> => {
        try {
            const newLabelIds = await todoListService.toggleLabel(itemId, labelId);
            setItems(prev => prev.map(i => i.id === itemId ? { ...i, label_ids: newLabelIds } : i));
        } catch (e: unknown) { setError(e instanceof Error ? e.message : 'Error'); }
    }, []);

    return {
        items, labels, persons, error,
        createItem, updateItem, deleteItem,
        createLabel, updateLabel, deleteLabel, toggleLabel,
        refetch: fetchItems,
    };
}
