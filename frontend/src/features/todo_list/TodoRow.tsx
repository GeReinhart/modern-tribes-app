        ...
        <div style={{ borderRadius: '8px', border: `1px solid ${theme.colors.border}`, marginBottom: '8px', overflow: 'hidden', backgroundColor: theme.colors.surface, opacity: isArchived ? 0.65 : 1 }}>
            ...
            {expanded && (
                <div style={{ padding: '0 14px 12px 14px', borderTop: `1px solid ${theme.colors.border}` }}>
                    {cardLabels.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', paddingTop: '10px' }}>
                            {cardLabels.map(label => (
                                <span key={label.id} style={{ padding: '2px 8px', borderRadius: '10px', border: `1px solid ${label.color}`, background: label.color, color: theme.colors.surface, fontSize: '11px', fontWeight: 600 }}>
                                    {label.name}
                                </span>
                            ))}
                        </div>
                    ...
                    <div style={{ paddingTop: '10px' }}>
                        {item.document_content_html
                            ...
                            : <span style={{ fontSize: 'var(--font-sm)', color: theme.colors.secondary, fontStyle: 'italic' }}>{t('features.todo.noNote')}</span>
                        }
                    </div>
                </div>
            )}
            ...
            {modalOpen && (
                <TodoItemModal
                    ...
                    canEdit={canEdit && !isArchived} isManager={isManager}
