...
export const tabConfigService = {
    get: (contextKey: string): Promise<TabConfigResponse> =>
        apiService.get<TabConfigResponse>(`/user-tab-configs/${encodeURIComponent(contextKey)}`),

    save: (contextKey: string, tabConfigs: TabWithConfig[]): Promise<TabConfigResponse> => {
        const payload: TabConfigItem[] = tabConfigs.map(({ key, visible, order, is_default }) => ({
