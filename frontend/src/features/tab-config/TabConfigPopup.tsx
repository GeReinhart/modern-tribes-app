...
function moveTab(tabs: TabWithConfig[], index: number, direction: -1 | 1): TabWithConfig[] {
...
function toggleVisible(tabs: TabWithConfig[], key: string): TabWithConfig[] {
    const updated = tabs.map(t => t.key === key ? { ...t, visible: !t.visible } : t);
    const hasDefault = updated.some(t => t.is_default && t.visible);
    if (!hasDefault) {
        const first = updated.find(t => t.visible);
        return updated.map(t => ({ ...t, is_default: first ? t.key === first.key : t.is_default }));
    }
    return updated;
}
...
export const TabConfigPopup: React.FC<TabConfigPopupProps> = ({ tabsWithConfig, onSave, onClose }) => {
    ...
    return (
        ...
        >
            <div
                style={{ backgroundColor: theme.colors.surface, border: `2px solid ${theme.colors.border}`, borderRadius: '12px', padding: 'var(--space-xl)', boxShadow: '0 16px 48px rgba(0,0,0,0.2)', width: '480px', maxWidth: '95vw', display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}
            ...
            >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-sm)' }}>
                    ...
                    {draft.map((tab, index) => (
                        <TabConfigRow
                            ...
                            onToggleVisible={() => setDraft(prev => toggleVisible(prev, tab.key))}
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-sm)', marginTop: 'var(--space-sm)' }}>
                    <ThemedButton variant="ghost" onClick={onClose}>{t('common.cancel')}</ThemedButton>
...
const TabConfigHeader: React.FC<TabConfigHeaderProps> = ({ theme, t }) => (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: 'var(--space-sm)', alignItems: 'center', paddingBottom: 'var(--space-xs)', borderBottom: `1px solid ${theme.colors.border}`, color: theme.colors.secondary, fontSize: 'var(--font-sm)', fontWeight: 600 }}>
        <span>{t('tabConfig.tab')}</span>
        <span style={{ textAlign: 'center', width: '72px' }}>{t('tabConfig.visible')}</span>
        <span style={{ textAlign: 'center', width: '64px' }}>{t('tabConfig.default')}</span>
...
const TabConfigRow: React.FC<TabConfigRowProps> = ({
    tab, index, total, theme, t, onMoveUp, onMoveDown, onToggleVisible, onSetDefault,
}) => {
    ...
    return (
        <div style={rowStyle}>
            <span style={{ color: tab.visible ? theme.colors.text : theme.colors.secondary, fontWeight: tab.is_default ? 600 : 400 }}>
                {tab.label}
            </span>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', width: '64px' }}>
                <input
                    ...
                    style={{ width: '16px', height: '16px', cursor: tab.visible ? 'pointer' : 'not-allowed', accentColor: theme.colors.primary }}
                />
            </div>
            ...
            <div style={{ display: 'flex', gap: '2px', width: '56px', justifyContent: 'flex-end' }}>
                <button
                    onClick={onMoveUp}
                    disabled={index === 0}
                    style={{ padding: '2px 4px', border: 'none', background: 'transparent', cursor: index === 0 ? 'not-allowed' : 'pointer', color: index === 0 ? theme.colors.secondary : theme.colors.text, borderRadius: '4px' }}
                ...
                <button
                    ...
                    style={{ padding: '2px 4px', border: 'none', background: 'transparent', cursor: index === total - 1 ? 'not-allowed' : 'pointer', color: index === total - 1 ? theme.colors.secondary : theme.colors.text, borderRadius: '4px' }}
