...
function uniquePersons(data: MyTasksResponse): PersonOption[] {
    ...
    for (const t of tasks) {
        if (t.assigned_person_id && t.assigned_person_name && !seen.has(t.assigned_person_id)) {
...
const DashboardTasksTab: React.FC = () => {
    ...
    const handleUpdate = useCallback(async (task: DashboardTask, patch: TaskPatch) => {
        await updateTask(task, patch);
        await refetch();
    }, [updateTask, refetch]);

    const handleToggleLabel = useCallback(async (task: DashboardTask, labelId: string) => {
    ...
    if (error) {
        return <div style={{ color: theme.colors.danger, fontSize: 'var(--font-sm)', padding: '12px 0' }}>{error}</div>;
    }
