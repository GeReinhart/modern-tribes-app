...
const TaskItemModal: React.FC<TaskItemModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (title.trim() && title.trim() !== value.title)
      patch.title = title.trim();
    if (size !== value.size) {
      if (size === null) patch.clear_size = true;
      else patch.size = size;
    }
    if (notes !== (value.document_content_html ?? ''))
      patch.document_content_html = notes;
        ...
        >
          <input
            ...
            style={{
              ...
              fontWeight: 600,
              border: canEdit ? `1px solid ${theme.colors.border}` : 'none',
              background: canEdit ? theme.colors.surface : 'transparent',
              padding: canEdit ? '6px 10px' : '6px 0',
            }}
            ...
            {canEdit ? (
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: value.document_content_html,
                }}
              />
            ) : (
              <div
