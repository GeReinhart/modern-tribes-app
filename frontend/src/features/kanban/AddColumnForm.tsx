...
const AddColumnForm: React.FC<AddColumnFormProps> = ({ onAdd }) => {
    ...
    return (
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '12px', border: `1px solid ${theme.colors.border}`, borderRadius: '10px', backgroundColor: theme.colors.surface }}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
                <button type="submit" disabled={!name.trim() || submitting}
                    style={{ ...compactBtn, background: !name.trim() || submitting ? theme.colors.border : theme.colors.primary, color: !name.trim() || submitting ? theme.colors.secondary : theme.colors.surface }}>
                    {t('features.kanban.addColumn')}
                </button>
                <button type="button" onClick={() => { setShow(false); setName(''); }}
