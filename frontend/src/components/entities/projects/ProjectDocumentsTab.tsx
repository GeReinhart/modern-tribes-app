        ...
        <div>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '16px' }}>
                ...
                {canEdit && (
                    <ThemedButton
                        variant="primary"
                        onClick={() => navigate(`/app/tribes/${tribeId}/projects/${projectId}/documents/new`)}
                    >
            ...
            {labels.length > 0 && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '16px', alignItems: 'center' }}>
                    ...
                    {labels.map(label => (
                        ...
                        >
                            {label.name} <span style={{ opacity: 0.6 }}>({label.usage_count})</span>
                        </button>
                    ))}
                </div>
            )}
            ...
            {error && (
                <div style={{ color: theme.colors.danger, fontSize: 'var(--font-sm)', padding: '8px' }}>
                    {error}
                </div>
            )}
            ...
            {!loading && documents.map((doc: ProjectDocumentSummary) => (
                <div
                    key={doc.id}
