...
import { Tribe, TribeCreate, TribeUpdate, TribeProjectInput, TribeProjectRelation } from '@/types/tribe.types.ts';
...
export const TribeForm: React.FC<TribeFormProps> = ({ tribe, mode, onSubmit, onCancel }) => {
    ...
    const [formData, setFormData] = useState<TribeCreate>({ name: '', document_id: null });
    ...
    const updateRelation = useCallback((projectId: string, relation: string) => {
        setProjectRows(prev =>
            prev.map(r => r.project_id === projectId ? { ...r, relation: relation as TribeProjectRelation } : r)
        );
    }, []);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setError(null);
        try {
            await onSubmit(mode === 'create' ? formData : { ...formData, status }, projectRows);
        } catch (err: any) {
            ...
            <div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end', marginBottom: '6px' }}>
                        ...
                        <button
                            ...
                            style={{
                                ...
                                background: showOnlySelected ? `${theme.colors.accent}18` : 'transparent',
                                color: showOnlySelected ? theme.colors.accent : theme.colors.text,
                                fontSize: '12px',
                                fontWeight: 500,
                                cursor: 'pointer',
                                whiteSpace: 'nowrap',
                        ...
                        >
                            {showOnlySelected ? `Selected (${projectRows.length})` : 'All projects'}
                        </button>
                    </div>
                )}
                ...
                <div style={{
                    ...
                    {filteredProjects.map(project => {
                        ...
                        const relationColor = row ? RELATION_COLORS[row.relation] : theme.colors.primary;
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    ...
                                    background: isSelected ? `${relationColor}12` : `${theme.colors.primary}05`,
                            ...
                            >
                                {!isReadOnly && (
                                    <input
                                        ...
                                        checked={isSelected}
                                        readOnly
                                        style={{ width: '15px', height: '15px', cursor: 'pointer', accentColor: relationColor, flexShrink: 0 }}
                                    />
                                )}
                                ...
                                {!isSelected && isReadOnly && (
                                    <span style={{ fontSize: '11px', color: theme.colors.secondary }}>—</span>
                                )}
                            </div>
                        );
                    })}
