...
interface UserFormProps {
    user?: User;
    mode: FormMode;
    onSubmit: (data: UserCreate | UserUpdate, representPersonIds: string[]) => Promise<void>;
    onCancel: () => void;
...
export const UserForm: React.FC<UserFormProps> = ({ user, mode, onSubmit, onCancel, initialRepresentPersonIds }) => {
    ...
    const filteredPersonsForRepresents = useMemo(() => {
        let list = persons;
        if (showOnlySelected) list = list.filter(p => representPersonIds.includes(p.id));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (mode === 'create') {
                ...
                await onSubmit({ ...formData, status } as UserUpdate, representPersonIds);
            }
        } catch (error) {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <ThemedSelect
                ...
                onChange={e => setFormData(prev => ({ ...prev, person_id: e || null }))}
                            ...
                            return (
                                <div
                                    ...
                                    style={{
                                        ...
                                        background: isSelected ? `${theme.colors.accent}18` : `${theme.colors.primary}05`,
                                ...
                                >
                                    {!isViewMode && (
                                        <input
                                            ...
                                            checked={isSelected}
                                            readOnly
                                            style={{ width: '15px', height: '15px', cursor: 'pointer', accentColor: theme.colors.accent, flexShrink: 0 }}
                                        />
                                    )}
