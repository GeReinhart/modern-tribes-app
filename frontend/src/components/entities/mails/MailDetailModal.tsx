...
export const MailDetailModal: React.FC<MailDetailModalProps> = ({ mail, onClose }) => {
    ...
    const row = (label: string, value: React.ReactNode) => (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
            <ThemedText variant="secondary" size="small">{label}</ThemedText>
            <div>{value}</div>
        </div>
    );

    return (
        <ThemedModal isOpen={!!mail} onClose={onClose} title={mail.subject} size="lg">
            <ModalBody>
                <div className="space-y-3">

                    {/* Meta */}
                    {mail.mail_type && row(t('admin.mails.mailType'), (
                        <ThemedBadge variant="secondary">{mail.mail_type}</ThemedBadge>
                    ))}
                    {row(t('monitoring.status'), <StatusBadge status={mail.status} />)}
                    {row(t('admin.mails.mailStatus'), <MailStatusBadge mailStatus={mail.mail_status} />)}
                    {row(t('admin.mails.plannedAt'), (
                        <ThemedText variant="text" size="small">
                            {new Date(mail.planned_at).toLocaleString()}
                        </ThemedText>
                    ))}
                    {mail.sent_at && row(t('admin.mails.sentAt'), (
                        <ThemedText variant="text" size="small">
                            {new Date(mail.sent_at).toLocaleString()}
                        </ThemedText>
                    ))}
                    {mail.created_by && row(t('monitoring.createdBy'), (
                        <ThemedText variant="text" size="small">{mail.created_by}</ThemedText>
                    ))}

                    {/* Recipients */}
                    <ThemedDivider variant="secondary" />
                    <ThemedText variant="primary" size="small" as="h4">{t('admin.mails.recipients')}</ThemedText>
                    {mail.recipient_emails.length === 0 ? (
                        <ThemedText variant="secondary" size="small">{t('admin.mails.noRecipients')}</ThemedText>
                    ) : (
                    ...
                    {/* Content */}
                    <ThemedDivider variant="secondary" />
                    <ThemedText variant="primary" size="small" as="h4">{t('admin.mails.content')}</ThemedText>
                    <div
                        style={{
