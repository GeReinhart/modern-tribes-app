...
export const ThemedSelect: React.FC<ThemedSelectProps> = ({
    ...
    return (
        <div className="w-full" ref={containerRef}>
            {label && (
                <label htmlFor={inputId} className="block text-sm font-medium mb-1" style={{ color: theme.colors[variant] }}>
                    {label}
                </label>
            )}
            <div className="relative">
                ...
                >
                    <span style={{ color: selectedLabel ? theme.colors.text : theme.colors.text, opacity: selectedLabel ? 1 : 0.5 }}>
                        {selectedLabel || placeholder}
                    </span>
                    <svg className="w-4 h-4 ml-2 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ color: theme.colors.text }}>
                        ...
                        <div className="max-h-52 overflow-auto">
                            {allowEmpty && (
                                <div
                                    ...
                                    style={{ backgroundColor: value === '' ? `${theme.colors[variant]}10` : 'transparent' }}
                                    onClick={() => handleSelect('')}
                                    onMouseEnter={(e) => { if (value !== '') e.currentTarget.style.backgroundColor = `${theme.colors[variant]}05`; }}
                                    onMouseLeave={(e) => { if (value !== '') e.currentTarget.style.backgroundColor = 'transparent'; }}
                                >
                                    <span style={{ color: theme.colors.text, opacity: 0.5 }}>{placeholder}</span>
                                </div>
                            )}
                            {filteredOptions.map((option) => {
                                ...
                                return (
                                    <div
                                        key={option.value}
                                        className="px-3 py-2 cursor-pointer"
                                        style={{ backgroundColor: isSelected ? `${theme.colors[variant]}10` : 'transparent' }}
                                        onClick={() => handleSelect(option.value)}
                                        onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.backgroundColor = `${theme.colors[variant]}05`; }}
