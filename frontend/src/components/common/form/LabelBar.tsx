        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', alignItems: 'center' }}>
            ...
            {labels.map(label => {
                ...
                if (editingId === label.id) {
                    return (
                        <div key={label.id} style={{ display: 'flex', flexDirection: 'column', gap: '6px', padding: '8px', border: `1px solid ${theme.colors.border}`, borderRadius: '8px', backgroundColor: theme.colors.surface }}>
                            <input
                                autoFocus
                                value={editName}
                                ...
                                style={{ padding: '4px 8px', borderRadius: '6px', fontSize: 'var(--font-xs)', border: `1px solid ${theme.colors.border}`, backgroundColor: theme.colors.surface, color: theme.colors.text, outline: 'none' }}
                            />
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                                {COLOR_PALETTE.map(c => (
                                    <button key={c} type="button" onClick={() => setEditColor(c)}
                                        style={{ width: '18px', height: '18px', borderRadius: '50%', backgroundColor: c, border: editColor === c ? `2px solid ${theme.colors.text}` : '2px solid transparent', cursor: 'pointer', padding: 0, flexShrink: 0 }} />
                                ))}
                            </div>
                            <div style={{ display: 'flex', gap: '4px' }}>
                                <button type="button" onClick={() => saveEdit(label)}
                                    style={{ flex: 1, padding: '3px 6px', fontSize: 'var(--font-xs)', borderRadius: '4px', backgroundColor: theme.colors.primary, color: theme.colors.surface, border: 'none', cursor: 'pointer' }}>
                                    {t('common.save')}
                                </button>
                                <button type="button" onClick={cancelEdit}
                                    style={{ padding: '3px 6px', fontSize: 'var(--font-xs)', borderRadius: '4px', backgroundColor: 'transparent', color: theme.colors.secondary, border: `1px solid ${theme.colors.border}`, cursor: 'pointer' }}>
                                    {t('common.cancel')}
                                </button>
                            </div>
                        </div>
                ...
                return (
                    <div key={label.id} style={{ display: 'flex', alignItems: 'center', gap: '2px', opacity: isUsed ? 1 : 0.5 }}>
                        <button type="button"
                            onClick={() => { if (isUsed) onFilter(isActive ? null : label.id); }}
                        ...
                        {canEditLabels && (
                            <>
                                ...
                                <button type="button" title={t('labels.delete')} onClick={() => onDelete(label.id)}
                                    style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', display: 'flex', alignItems: 'center', opacity: 0.4 }}>
